...
namespace DatabaseHandler.Extensions
{
    public static class DbContextExtension
    {
        ...
        public static IServiceCollection AddEntityFramework<T>(this IServiceCollection services, Action<IServiceProvider, DbContextOptionsBuilder> optionBuilder, int poolSize = 1024) where T : DbContext<T>
        {
            services.TryAddSingleton<Snowflake>();
            services.AddCurrentUser();
            services.AddFactory<T>(optionBuilder, poolSize);
        ...
        public static IServiceCollection AddFactory<T>(this IServiceCollection services, Action<IServiceProvider, DbContextOptionsBuilder> optionBuilder, int poolSize = 1024) where T : DbContext<T>
        {
            services.AddPooledDbContextFactory<T>((provider, config) =>
            {
