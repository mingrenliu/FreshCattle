...
public static class BuiltInConverters
{
    ...
    /// 尝试获取内置转换器。
    /// </summary>
    public static bool TryGetConverter(Type origin, [NotNullWhen(true)] out ExcelConverter? converter)
    {
        if (_cache.TryGetValue(origin, out converter))
            return true;
        var type = Nullable.GetUnderlyingType(origin) ?? origin;
        if (_cache.TryGetValue(type, out converter!))
        {
            _cache[origin] = converter; // 缓存 Nullable<T> 的转换器
            return true;
        }
    ...
    /// 尝试获取内置转换器。
    /// </summary>
    public static bool TryGetConverter<T>([NotNullWhen(true)] out ExcelConverter<T>? converter)
    {
        if (TryGetConverter(typeof(T), out var baseConverter) && baseConverter is ExcelConverter<T> typed)
        {
            converter = typed;
            return true;
        }
    ...
    public static ExcelConverter<T>? GetConverter<T>()
    {
